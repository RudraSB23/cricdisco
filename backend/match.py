...
def simulate_match(team_a: Team, team_b: Team, overs: int = 5) -> MatchResult:
    ...
    print(f"\n=== Second Innings: {team_b.manager_name} chasing {target} ===\n")
    ...
    if innings_a.runs > innings_b.runs:
        ...
        if wickets_left <= 0:
            # chased with last wicket
            margin = "wins by 1 wicket"
        else:
            margin = f"wins by {wickets_left} wickets"
...
def simulate_innings(
    ...
    for over in range(overs):
        ...
        print(f"Over {over + 1}: {bowler.name} to {batting_order[striker_index].name}")
        ...
        for ball_in_over in range(6):
            ...
            if outcome["is_wicket"]:
                wickets += 1
                print(f"{desc_prefix}{outcome['description']}  (Wicket!)")
                # New batter in at striker position
                ...
                print(f"{desc_prefix}{outcome['description']}")
        ...
        # Pause between overs
        _ = input("Press Enter to continue to the next over...")
